  ...
  return (
    <div className="chat-layout">
      <section className="paper-card chat-card">
        ...
        <form onSubmit={send}><textarea rows={3} spellCheck value={draft} onChange={(event) => setDraft(event.target.value)} placeholder="Try: When is my next assignment?" /><button type="submit" disabled={thinking}>{thinking ? "Checking…" : "Ask assistant"}</button></form>
      </section>
    ...
    <div className="social-hub">
      ...
      {view === "feed" ? <div className="social-layout">
        <main className="paper-card social-feed-card">
          <form className="status-compose" onSubmit={publish}><div className="compose-person"><img src={persona.image} alt="" /><div><strong>Share an update</strong><span>{statusLine}</span></div></div><textarea rows={3} spellCheck value={body} onChange={(event) => setBody(event.target.value)} placeholder="Share class progress, a question, a project, or a study win…" />{media && <div className="image-preview">{mediaType === "video" ? <video src={media} controls /> : <img src={media} alt="Selected upload preview" />}<button type="button" onClick={() => { setMedia(""); setMediaType(""); }}>Remove selected media</button></div>}<footer><label className="upload-button">Upload picture or video<input type="file" accept="image/*,video/*" onChange={(event) => readMedia(event.target.files?.[0])} /></label><select aria-label="Choose post audience" value={audience} onChange={(event) => setAudience(event.target.value)}>{rules.availableAudiences.map((item) => <option key={item} value={item}>{item[0].toUpperCase() + item.slice(1)}</option>)}</select><button type="submit">Post update</button></footer></form>
