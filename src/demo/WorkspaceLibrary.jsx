...
function SourcesPanel({ persona }) {
  ...
  return (
    <div className="workspace-panel-stack">
      <section className="paper-card library-header-card"><div><NotebookLabel>{persona.id === "professor" ? "RESEARCH & TEACHING LIBRARY" : "NOTES & SOURCE LIBRARY"}</NotebookLabel><h1>Keep the thought, the source, and the class together.</h1><p>Notes and sources are searchable by the AI memory demo. Source coaching keeps citation details visible instead of scattered across browser tabs.</p></div><div className="segmented-control"><button className={view === "notes" ? "is-active" : ""} type="button" onClick={() => setView("notes")}>Notes</button><button className={view === "sources" ? "is-active" : ""} type="button" onClick={() => setView("sources")}>Sources</button><button className={view === "documents" ? "is-active" : ""} type="button" onClick={() => setView("documents")}>Documents</button></div></section>
