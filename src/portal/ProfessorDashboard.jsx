...
const NAV_GROUPS = [
  { label: "Teach", items: [["overview", "Overview"], ["classes", "Class library"], ["semester", "Syllabus & calendar"], ["digital-literacy", "Digital Literacy Class"], ["templates", "Assignment templates"]] },
...
function Classes({ onBuild, classes, divisionScope = null, onSaveAccess, accessBusyCourse, onSaveLibrary, libraryBusyCourse }) {
  ...
  return <section className="dashboard-card">
    <div className="dashboard-card-heading">
      <div>
        <span className="portal-kicker">COURSE BUILDER LIBRARY</span>
        <h1>Your classes, organized.</h1>
        <p>{divisionScope === "k12" ? "Publish Early Prep classes to enrolled students and keep the school audience separate. Commercial library and marketplace tools stay unavailable." : "Publish once, then control student enrollment, automatic assignment, and Alex B. Morrison Library/Bookstore visibility independently."}</p>
      </div>
      <button type="button" onClick={() => onBuild(null)}>Create another class</button>
    </div>
    <div className="class-library-controls professor-library-controls">
      <label>Search classes<input type="search" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Code, title, or subject" /></label>
    ...
    {classes.length === 0 ? (
      <div className="empty-class-list"><strong>Your first class starts in Course Builder.</strong><p>Create the course, review its student experience, and publish when it is ready.</p><button type="button" onClick={() => onBuild(null)}>Open Course Builder</button></div>
    ) : visible.length === 0 ? (
      <div className="empty-class-list"><strong>No classes match these filters.</strong><button type="button" onClick={() => { setQuery(""); setDivision(divisionScope || "all"); setStatus("all"); }}>Clear filters</button></div>
    ) : (
    ...
    <div className="class-publishing-note"><strong>One governed course package</strong><span>Course Builder controls the live state and owns the approved package. Student enrollment and universal assignment reference it directly; commercial distribution is available only in the University division.</span></div>
  </section>;
}
...
function AttendancePanel({ classes = EDUCATOR_CLASSES }) {
...
export default function ProfessorDashboard({ profile, session, divisionScope = null, onHome, onBuild, onAdmin }) {
  ...
  const allTeachingClasses = courseLibrary.length ? courseLibrary : EDUCATOR_CLASSES;
  const teachingClasses = divisionScope ? allTeachingClasses.filter((course) => course.division === divisionScope) : allTeachingClasses;
    ...
    <div className={`professor-dashboard-page ${accountSettings.showDescriptions ? "" : "is-description-light"}`}>
      ...
      <div className="student-dashboard-shell professor-dashboard-shell">
        <aside className="student-dashboard-sidebar professor-sidebar">
          <div className="student-sidebar-profile"><span>{displayName.slice(0, 1).toUpperCase()}</span><div><strong>{displayName}</strong><small>{teachingClasses.length} class{teachingClasses.length === 1 ? "" : "es"} · {pendingRequestCount} waiting</small></div></div>
        ...
        <main className="student-dashboard-main professor-dashboard-main">
          ...
          {tab === "overview" && <Overview setTab={setTab} classes={teachingClasses} enrollmentRequests={teachingEnrollmentRequests} />}
          {tab === "classes" && <Classes onBuild={onBuild} classes={teachingClasses} divisionScope={divisionScope} onSaveAccess={saveCourseAccess} accessBusyCourse={accessBusyCourse} onSaveLibrary={saveLibraryListing} libraryBusyCourse={libraryBusyCourse} />}
          ...
          {tab === "templates" && <AssignmentTemplateWorkspace mode="professor" session={session} track={divisionScope || "university"} classes={teachingClasses} />}
