...
function Overview({ setTab }) {
  return <div className="professor-panel-stack"><section className="professor-welcome-card"><div><span>FALL 2026 · EDUCATOR WORKSPACE</span><h1>University and K–12 classes, one place to run them.</h1><p>Start with any email. School verification is optional unless you want a verified affiliation badge.</p></div><button type="button" onClick={() => setTab("classes")}>Open classes</button></section><section className="student-stat-grid professor-stat-grid"><article><span>Active classes</span><strong>3</strong><button type="button" onClick={() => setTab("classes")}>Manage</button></article><article><span>Enrolled students</span><strong>93</strong><button type="button" onClick={() => setTab("students")}>Open roster</button></article><article><span>Link requests</span><strong>3</strong><small>Educator approval needed</small></article><article><span>Grades to finish</span><strong>2</strong><button type="button" onClick={() => setTab("grades")}>Open gradebook</button></article></section><section className="professor-dashboard-columns"><article className="dashboard-card"><span className="portal-kicker">ACCOUNT LINKING</span><h2>Students waiting</h2>{INITIAL_ROSTER.filter((item) => item.status === "pending").map((student) => <div className="professor-alert-row" key={student.id}><span>{student.name}</span><strong>{student.account}</strong></div>)}<button type="button" onClick={() => setTab("students")}>Review links</button></article><article className="dashboard-card"><span className="portal-kicker">SCHOOL AFFILIATION</span><h2>Unverified does not mean blocked.</h2><p>Your classes can stay live and plans remain available. Verification adds a badge beside the school name after manual review.</p><button type="button" onClick={() => setTab("verification")}>Request verification</button></article></section></div>;
}

function Classes({ onBuild }) {
  const [classes, setClasses] = useState(EDUCATOR_CLASSES);
  function togglePublish(id) { setClasses(classes.map((course) => course.id === id ? { ...course, published: !course.published } : course)); }
  return <section className="dashboard-card"><div className="dashboard-card-heading"><div><span className="portal-kicker">CLASS PUBLISHING</span><h1>Your classes</h1><p>Public listings work for verified and unverified educators. Course content still requires approved enrollment.</p></div><button type="button" onClick={onBuild}>Create another class</button></div><div className="professor-class-grid">{classes.map((course) => <article key={course.id}><div><span>{course.code} · {course.division === "k12" ? "K–12" : "University"}</span><strong>{course.title}</strong><small>{course.term} · {course.students} students</small></div><div className="professor-class-status"><span className={course.published ? "is-live" : "is-draft"}>{course.published ? "Public listing live" : "Draft only"}</span><span className="educator-verification-badge is-unverified">Affiliation unverified</span><label><input type="checkbox" checked={course.published} onChange={() => togglePublish(course.id)} />Listed in student search</label></div><footer><button type="button" onClick={onBuild}>Open course builder</button>{course.published && <a href={`#/students/${course.division}?course=${course.id}`}>View as a student</a>}</footer></article>)}</div><div className="class-publishing-note"><strong>What the public sees</strong><span>School, code, title, educator, subject, term, schedule, summary, seat availability, and verification badge. No roster, lessons, files, messages, or grades.</span></div></section>;
}
