...
import { listAccountAudit, listEducatorVerificationRequests, reviewAccountAudit, reviewEducatorVerification } from "./portalService.js";
...
function AccountAuditCard({ account, onReview, busy }) {
  const statusLabel = String(account.audit_status || "unreviewed").replaceAll("_", " ");
  return <article className="admin-verification-card"><header><div><span>{account.role || "account"}</span><h3>{account.full_name || "Unnamed account"}</h3><small>{account.email || account.user_id}</small></div><strong className={`roster-status is-${account.audit_status}`}>{statusLabel}</strong></header><dl><div><dt>Created</dt><dd>{new Date(account.created_at).toLocaleString()}</dd></div><div><dt>Last sign in</dt><dd>{account.last_active_at ? new Date(account.last_active_at).toLocaleString() : "No sign-in recorded"}</dd></div><div><dt>Meaningful activity</dt><dd>{account.meaningful_activity_at ? new Date(account.meaningful_activity_at).toLocaleString() : "None recorded"}</dd></div><div><dt>Flagged</dt><dd>{account.inactive_flagged_at ? new Date(account.inactive_flagged_at).toLocaleString() : "Not flagged"}</dd></div></dl><footer><button type="button" disabled={busy} onClick={() => onReview(account.user_id, "test_account")}>Mark test account</button><button type="button" disabled={busy} onClick={() => onReview(account.user_id, "active")}>Reactivate</button><button className="primary" type="button" disabled={busy} onClick={() => onReview(account.user_id, "confirmed_user")}>Confirm real user</button></footer></article>;
}
...
export default function PlatformAdminDashboard({ onHome, onEducatorPortal }) {
  ...
  const [accountFilter, setAccountFilter] = useState("inactive_review");
  ...
  async function loadAccounts(nextFilter = accountFilter) {
    setLoading(true); setError("");
    const { data, error: loadError } = await listAccountAudit(nextFilter);
    if (loadError) setError(loadError.message || "Account audit rows could not be loaded.");
  ...
  async function reviewAccount(userId, status) {
    setBusyUser(userId); setError("");
    const { error: reviewError } = await reviewAccountAudit(userId, status);
    if (reviewError) setError(reviewError.message || "The account audit status could not be updated.");
  ...
  return (
    <div className="platform-admin-page">
      <header className="dashboard-topbar"><button className="dashboard-brand" type="button" onClick={onHome}><BrandLogo size={38} tagline="Master admin" /></button><div className="dashboard-top-actions"><LiveDateTime /><button type="button" className={view === "verification" ? "is-active" : ""} onClick={() => setView("verification")}>Verification queue</button><button type="button" className={view === "accounts" ? "is-active" : ""} onClick={() => { setView("accounts"); loadAccounts(); }}>Account audit</button><button type="button" className={view === "messages" ? "is-active" : ""} onClick={() => { loadReports(); setView("messages"); }}>Messages {reports.filter((item) => item.status === "new").length > 0 && `(${reports.filter((item) => item.status === "new").length})`}</button><button type="button" onClick={onEducatorPortal}>Educator workspace</button><button className="primary" type="button" onClick={() => view === "messages" ? loadReports() : view === "accounts" ? loadAccounts() : load()}>{view === "messages" ? "Refresh messages" : view === "accounts" ? "Refresh accounts" : "Refresh queue"}</button></div></header>
      <main className="platform-admin-main">
        <section className="admin-hero"><span className="portal-kicker">MASTER ADMIN</span><h1>{view === "messages" ? "Extraction reports and user messages." : view === "accounts" ? "Separate real use from empty test accounts." : "University and K–12, clearly divided."}</h1><p>{view === "messages" ? "Reports sent from the syllabus review screen include the account or guest name, source, stage, time, and issue." : view === "accounts" ? "Inactive accounts stay recoverable. Review the signal, hide test accounts from discovery, or reactivate them without deleting their work." : "Review school-affiliation requests and manage each education division from one admin workspace."}</p></section>
        {view === "messages" ? <section className="dashboard-card admin-message-inbox"><div className="dashboard-card-heading"><div><span className="portal-kicker">ADMIN MESSAGES</span><h2>{reports.length} report{reports.length === 1 ? "" : "s"}</h2></div><span>{reports.filter((item) => item.status === "new").length} new</span></div>{reports.length === 0 ? <div className="admin-empty-state"><h2>No extraction reports yet.</h2><p>Reports from the syllabus review button will appear here on this device.</p></div> : <div className="admin-message-list">{reports.map((report) => <article key={report.id} className={report.status === "new" ? "is-new" : ""}><header><div><strong>{report.reporter?.name || "Guest"}</strong><span>{report.reporter?.accountType || "Guest"} · {new Date(report.createdAt).toLocaleString()}</span></div><b>{report.status}</b></header><dl><div><dt>Source</dt><dd>{report.sourceFile}</dd></div><div><dt>Stage</dt><dd>{report.stage}</dd></div></dl><p>{report.issue}</p><footer><button type="button" onClick={() => updateReport(report.id, report.status === "resolved" ? "new" : "resolved")}>{report.status === "resolved" ? "Reopen" : "Mark resolved"}</button></footer></article>)}</div>}</section> : view === "accounts" ? <section className="dashboard-card"><div className="dashboard-card-heading"><div><span className="portal-kicker">ACCOUNT AUDIT</span><h2>Recoverable account classifications</h2></div><label>Show<select value={accountFilter} onChange={(event) => { const next = event.target.value; setAccountFilter(next); loadAccounts(next); }}><option value="inactive_review">Needs review</option><option value="unreviewed">New and unreviewed</option><option value="active">Active</option><option value="confirmed_user">Confirmed real users</option><option value="test_account">Test accounts</option><option value="all">All accounts</option></select></label></div>{error && <div className="portal-form-error" role="alert">{error}</div>}{loading ? <p>Loading account audit…</p> : accounts.length === 0 ? <div className="admin-empty-state"><h2>No accounts in this group.</h2><p>The audit worker flags accounts with no meaningful activity. It never deletes them.</p></div> : <div className="admin-verification-grid">{accounts.map((account) => <AccountAuditCard key={account.user_id} account={account} onReview={reviewAccount} busy={busyUser === account.user_id} />)}</div>}</section> : <><section className="student-stat-grid admin-stat-grid"><article><span>Pending review</span><strong>{stats.pending}</strong><small>Manual decision needed</small></article><article><span>University requests</span><strong>{stats.university}</strong><small>College and university affiliations</small></article><article><span>K–12 requests</span><strong>{stats.k12}</strong><small>School and district affiliations</small></article><article><span>Admin messages</span><strong>{reports.filter((item) => item.status === "new").length}</strong><small>Extraction reports waiting</small></article></section><section className="dashboard-card"><div className="admin-division-tabs" role="tablist" aria-label="Education division"><button className={division === "university" ? "is-active" : ""} type="button" onClick={() => setDivision("university")}>University division</button><button className={division === "k12" ? "is-active" : ""} type="button" onClick={() => setDivision("k12")}>K–12 division</button></div>{error && <div className="portal-form-error" role="alert">{error}</div>}{loading ? <p>Loading verification queue…</p> : filtered.length === 0 ? <div className="admin-empty-state"><h2>No requests in this division.</h2><p>New teacher ID submissions will appear here for manual review.</p></div> : <div className="admin-verification-grid">{filtered.map((request) => <RequestCard key={request.user_id} request={request} onReview={review} onOpenDocument={openDocument} busy={busyUser === request.user_id} />)}</div>}</section></>}
      </main>
    </div>
  );
