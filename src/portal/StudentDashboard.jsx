...
export default function StudentDashboard({ profile, session, track = "university", onHome, onProfessorPortal }) {
  ...
  return <div className={`student-dashboard-page ${track === "k12" ? "is-k12" : ""}`}><header className="dashboard-topbar"><button className="dashboard-brand" type="button" onClick={onHome}><BrandLogo size={38} tagline={`${copy.shortLabel} student`} /></button><span className="sample-workspace-badge">Sample workspace · not an official grade record</span><div className="dashboard-top-actions"><button type="button" onClick={onProfessorPortal}>Educator portal</button><button type="button" onClick={() => setTourStep(0)}>Take the tour</button><button className="primary" type="button" onClick={() => setTab("assignments")}>Open assignments</button></div></header><div className="student-dashboard-shell"><aside className="student-dashboard-sidebar"><div className="student-sidebar-profile"><span>{displayName.slice(0, 1).toUpperCase()}</span><div><strong>{displayName}</strong><small>{track === "k12" ? "Example High School" : "Example University"}</small></div></div><nav aria-label={`${copy.shortLabel} student dashboard`}>{TABS.map(([id, label]) => <button className={tab === id ? "is-active" : ""} type="button" key={id} onClick={() => setTab(id)}>{label}{id === "grades" && <i>2</i>}</button>)}</nav><div className="student-sidebar-points"><span>LEVEL 8</span><strong>1,645 points</strong><div><i /></div><small>355 points to level 9</small></div></aside><main className="student-dashboard-main">{tab === "overview" && <OverviewPanel name={displayName} onTab={setTab} classes={classes} track={track} />}{tab === "classes" && <ClassesPanel classes={classes} track={track} />}{tab === "assignments" && <AssignmentTemplateWorkspace mode="student" session={session} track={track} classes={classes} />}{tab === "grades" && <GradesPanel classes={classes} rows={rows} track={track} />}{tab === "notes" && <NotesPanel classes={classes} track={track} />}{tab === "life" && <StudentLifePanel groups={groups} initialPosts={posts} track={track} />}{tab === "messages" && <MessagesPanel track={track} />}{tab === "page" && <StudentPagePanel name={profile?.full_name || displayName} track={track} />}{tab === "opportunities" && <OpportunitiesPanel track={track} />}</main></div><DashboardTour step={tourStep} setStep={setTourStep} /></div>;
}
