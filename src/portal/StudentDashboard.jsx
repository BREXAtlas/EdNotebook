...
const TABS = [
  ["overview", "Overview"], ["classes", "Classes"], ["grades", "Grades"], ["notes", "Notes"],
  ["life", "Student life"], ["messages", "Messages"], ["page", "My page"], ["opportunities", "Opportunities"],
];
...
function DashboardTour({ step, setStep }) {
  if (step === null) return null;
  const [title, description] = TOUR[step];
  return <div className="dashboard-tour-backdrop" role="dialog" aria-modal="true" aria-labelledby="dashboard-tour-title"><div className="dashboard-tour-card"><span>TOUR · {step + 1} OF {TOUR.length}</span><h2 id="dashboard-tour-title">{title}</h2><p>{description}</p><div><button type="button" onClick={() => setStep(null)}>Close</button><button type="button" onClick={() => setStep(step === TOUR.length - 1 ? null : step + 1)}>{step === TOUR.length - 1 ? "Finish" : "Next"}</button></div></div></div>;
}
...
function NotesPanel({ classes, track }) {
  ...
  const [course, setCourse] = useState(classes[0].code);
...
function MessagesPanel({ track }) {
  ...
  return <div className="student-message-layout"><section className="dashboard-card"><span className="portal-kicker">DEVICE MESSAGES</span><h1>Quick conversations for this session.</h1><p>University and K–12 conversations use separate stores. This preview clears when you delete it or end this browser session.</p><div className="device-message-thread">{messages.length ? messages.map((item) => <article key={item.id}><strong>{item.sender}</strong><p>{item.body}</p><small>{new Date(item.createdAt).toLocaleTimeString()}</small></article>) : <p>No messages in this session.</p>}</div><form className="device-message-form" onSubmit={send}><textarea rows={3} value={body} onChange={(event) => setBody(event.target.value)} placeholder="Write a message…" /><button type="submit">Send on this device</button></form><button className="clear-message-button" type="button" onClick={clear}>Clear this session</button></section><aside className="dashboard-card"><span className="portal-kicker">OPTIONAL SYNC</span><h2>Keep messages across devices.</h2><p>Cloud sync is an optional paid storage feature. Class access and educator communication do not depend on buying it.</p><strong>$2.99 / month with Student Plus</strong></aside></div>;
}

function StudentPagePanel({ name, track }) {
  const key = `ednotebook-${track}-student-page`;
  const [bio, setBio] = useState(() => readStorage(window.localStorage, key, {}).bio || (track === "k12" ? "Learning, building, and getting a little better each week." : "Biology student building practical research and digital literacy skills."));
  const [notice, setNotice] = useState("");
  function save() { window.localStorage.setItem(key, JSON.stringify({ bio, track })); setNotice("Page saved to this device preview."); }
  return <div className="student-page-builder"><section className="dashboard-card"><span className="portal-kicker">MY {track === "k12" ? "SCHOOL" : "STUDENT"} PAGE</span><h1>A page for the work and interests you choose.</h1><p>Your {track === "k12" ? "K–12" : "university"} profile is separate from the other social network. Class enrollment unlocks the basic template.</p><label>Bio<textarea rows={4} value={bio} onChange={(event) => setBio(event.target.value)} /></label><div className="interest-field-grid"><label>Graduation year<input placeholder="2028" /></label><label>Interests<input placeholder="Robotics, writing, soccer" /></label></div><label>YouTube or project link<input type="url" placeholder="https://…" /></label><button type="button" onClick={save}>Save page</button>{notice && <div className="portal-form-notice">{notice}</div>}</section><aside className="student-public-page"><span>{track === "k12" ? "EXAMPLE HIGH SCHOOL" : "EXAMPLE UNIVERSITY"}</span><h2>{name}</h2><p>{bio}</p><div><strong>1,645</strong><span>learning points</span></div><article><strong>Current project</strong><span>{track === "k12" ? "Living Systems model" : "Cell structure study guide"}</span></article></aside>{track === "k12" && <section className="dashboard-card student-transfer-card"><span className="portal-kicker">AFTER HIGH SCHOOL</span><h2>Carry learning memory forward.</h2><p>When you move to the university portal, completed work history, preferences, and learning memory can transfer. Your K–12 posts, groups, messages, and school profile stay in the K–12 space.</p><button type="button">Preview university transfer</button></section>}<section className="professor-plan-strip">{STUDENT_PRICING.map((plan) => <article key={plan.name}><span>{plan.name}</span><strong>{plan.price}</strong><p>{plan.description}</p></article>)}</section></div>;
}
...
export default function StudentDashboard({ profile, track = "university", onHome, onProfessorPortal }) {
  const [tab, setTab] = useState("overview");
  const [tourStep, setTourStep] = useState(null);
  const copy = getTrack(track);
  const classes = track === "k12" ? K12_STUDENT_CLASSES : STUDENT_CLASSES;
  const rows = track === "k12" ? K12_GRADE_ROWS : GRADE_ROWS;
  const groups = track === "k12" ? K12_STUDENT_GROUPS : STUDENT_GROUPS;
  const posts = track === "k12" ? K12_COMMUNITY_POSTS : COMMUNITY_POSTS;
  const displayName = useMemo(() => profile?.full_name?.split(" ")[0] || (track === "k12" ? "Jordan" : "Maya"), [profile?.full_name, track]);
  return <div className={`student-dashboard-page ${track === "k12" ? "is-k12" : ""}`}><header className="dashboard-topbar"><button className="dashboard-brand" type="button" onClick={onHome}><BrandLogo size={38} tagline={`${copy.shortLabel} student`} /></button><span className="sample-workspace-badge">Sample workspace · not an official grade record</span><div className="dashboard-top-actions"><button type="button" onClick={onProfessorPortal}>Educator portal</button><button type="button" onClick={() => setTourStep(0)}>Take the tour</button><button className="primary" type="button" onClick={() => setTab("classes")}>Open classes</button></div></header><div className="student-dashboard-shell"><aside className="student-dashboard-sidebar"><div className="student-sidebar-profile"><span>{displayName.slice(0, 1).toUpperCase()}</span><div><strong>{displayName}</strong><small>{track === "k12" ? "Example High School" : "Example University"}</small></div></div><nav aria-label={`${copy.shortLabel} student dashboard`}>{TABS.map(([id, label]) => <button className={tab === id ? "is-active" : ""} type="button" key={id} onClick={() => setTab(id)}>{label}{id === "grades" && <i>2</i>}</button>)}</nav><div className="student-sidebar-points"><span>LEVEL 8</span><strong>1,645 points</strong><div><i /></div><small>355 points to level 9</small></div></aside><main className="student-dashboard-main">{tab === "overview" && <OverviewPanel name={displayName} onTab={setTab} classes={classes} track={track} />}{tab === "classes" && <ClassesPanel classes={classes} track={track} />}{tab === "grades" && <GradesPanel classes={classes} rows={rows} track={track} />}{tab === "notes" && <NotesPanel classes={classes} track={track} />}{tab === "life" && <StudentLifePanel groups={groups} initialPosts={posts} track={track} />}{tab === "messages" && <MessagesPanel track={track} />}{tab === "page" && <StudentPagePanel name={profile?.full_name || displayName} track={track} />}{tab === "opportunities" && <OpportunitiesPanel track={track} />}</main></div><DashboardTour step={tourStep} setStep={setTourStep} /></div>;
}
