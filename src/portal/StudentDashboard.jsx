...
const TABS = [
  ["overview", "Overview"], ["classes", "Classes"], ["live", "Live class updates"], ["scanner", "Syllabus scanner"], ["assignments", "Assignments"], ["grades", "Grades"], ["notes", "Notes & sources"],
  ["engagement", "Points & groups"], ["life", "Community"], ["rooms", "Study rooms"], ["friends", "Find friends"], ["messages", "Messages"], ["page", "My page"], ["opportunities", "Opportunities"],
  ["demo", "Brooke's demo"],
  ["settings", "Settings"],
];
...
function tabFromRoute(fallback = "overview") {
  ...
  return TAB_IDS.has(requested) && requested !== "demo" ? requested : fallback;
}
...
const TOUR = [
  ["Your term at a glance", "Start here for classes, grades, points, deadlines, and anything that needs attention.", "overview"],
  ["Every class in one list", "Open Classes to see your educator, progress, next work, and protected course content.", "classes"],
  ["The same grade scale", "Grades mirrors the categories and weights your educator publishes.", "grades"],
  ["The right community", "Class and school conversations stay in the education space where they started.", "life"],
  ["Private tools stay private", "Device messages clear with this browser session. You choose what appears on your page.", "settings"],
];
...
function DashboardTour({ step, setStep, showOnSignIn, onShowOnSignIn, onNavigate }) {
  useEffect(() => {
    if (step === null) return;
...
function StudentLifePanelV2({ groups, initialPosts, track, accountSettings, storageScope }) {
  ...
  const [sharePoints, setSharePoints] = useState(true);
  const [followingGuide, setFollowingGuide] = useState(true);
...
function StudentPagePanel({ name, track, userId, storageScope, accountSettings, onSettingsChange }) {
  ...
  async function save() {
    ...
    if (!error && isSupabaseConfigured && supabase) await supabase.rpc("record_account_activity", { p_event: "student_page_saved" });
...
export default function StudentDashboard({ profile, session, track = "university", onHome, onProfessorPortal }) {
  ...
  useEffect(() => {
    const syncTab = () => {
      const requested = tabFromRoute(tab);
      if (requested !== "demo") {
        setDemoMode(false);
  ...
  function setTab(nextTab, { updateRoute = true } = {}) {
    setTabState(nextTab);
    if (updateRoute) saveTabToRoute(nextTab);
  }

  function chooseTab(nextTab, options) {
    if (nextTab === "demo") {
      setDemoMode(true);
      setTab("overview", { updateRoute: false });
      return;
    }
    if (nextTab === "settings") setDemoMode(false);
    setTab(nextTab, options);
  }
    ...
    <div className={`student-dashboard-page ${track === "k12" ? "is-k12" : ""} ${accountSettings.showDescriptions ? "" : "is-description-light"}`}>
      <header className="dashboard-topbar">
        ...
        <div className="dashboard-top-actions">
          ...
          <FeatureFinder audience={track} onSelectTab={chooseTab} />
      ...
      <div className="student-dashboard-shell">
        <aside className="student-dashboard-sidebar">
          ...
          <label className="dashboard-mobile-feature-picker">Go to a feature<select value={demoMode ? "demo" : tab} onChange={(event) => chooseTab(event.target.value)}>{TABS.map(([id, label]) => <option value={id} key={id}>{label}</option>)}</select></label>
        ...
        <main className="student-dashboard-main">
          ...
          {tab === "live" && <LiveCourseUpdates mode="student" />}
          {tab === "scanner" && <StudentSyllabusScanner track={track} />}
          ...
          {tab === "engagement" && <EngagementPoints mode="student" session={session} classes={classes} />}
          ...
          {tab === "rooms" && <LiveLearningRooms mode="student" session={session} />}
          ...
          {tab === "settings" && <AccountSettings scope={settingsScope} accountType="student" settings={accountSettings} onSettingsChange={applyAccountSettings} authenticated={Boolean(session?.user)} accountEmail={session?.user?.email || ""} accountNumber={profile?.account_number || ""} educationTrack={track} />}
        </main>
      </div>
      <DashboardTour step={tourStep} setStep={setTourStep} showOnSignIn={showTourOnSignIn} onShowOnSignIn={updateTourPreference} onNavigate={chooseTab} />
    </div>
  );
}
