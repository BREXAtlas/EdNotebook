...
export default function ShareEdNotebook({ buttonLabel = "Share EdNotebook", className = "", targetPath = "#/students/university" }) {
  ...
  return <>
    ...
    {open && <div className="share-ednotebook-overlay" role="dialog" aria-modal="true" aria-labelledby="share-ednotebook-title"><section className="share-ednotebook-modal"><header><BrandLogo size={40} tagline="Fun, connected learning" /><button type="button" onClick={() => setOpen(false)} aria-label="Close sharing options">×</button></header><div><span className="portal-kicker">INVITE YOUR PEOPLE</span><h2 id="share-ednotebook-title">Learn together on EdNotebook.</h2><p>Share the student page with friends, classmates, or professors. They can create a free account and find their university, classes, and people.</p></div><img src="/ednotebook-share-card-v2.png" alt="EdNotebook invitation card with the website and an invitation to join free" /><div className="share-primary-actions"><button className="primary" type="button" onClick={sharePage}>Open device sharing</button><a href="/ednotebook-share-card-v2.png" download="ednotebook-invitation.png">Download share graphic</a></div><div className="share-social-options" aria-label="Social sharing links"><a href={`https://www.facebook.com/sharer/sharer.php?u=${encodedUrl}`} target="_blank" rel="noreferrer">Facebook</a><a href={`https://twitter.com/intent/tweet?text=${encodedText}`} target="_blank" rel="noreferrer">X / Twitter</a><a href={`https://www.linkedin.com/sharing/share-offsite/?url=${encodedUrl}`} target="_blank" rel="noreferrer">LinkedIn</a><a href={`https://wa.me/?text=${encodedText}`} target="_blank" rel="noreferrer">WhatsApp</a><a href={`mailto:?subject=${encodeURIComponent(SHARE_TITLE)}&body=${encodedText}`}>Email</a></div><label>Share link<div><input readOnly value={pageUrl} onFocus={(event) => event.target.select()} /><button type="button" onClick={copyLink}>Copy share link</button></div></label>{notice && <div className="portal-form-notice" role="status">{notice}</div>}</section></div>}
  </>;
}
