...
test("canonical unit links remain inside the governed EdNotebook embed contract", () => {
  const url = new URL(buildCanonicalUnitUrl({
    assignment: {
  ...
  assert.equal(url.searchParams.get("ednotebook_origin"), "https://ednotebook.com");
});
...
test("progress messages require the canonical origin, frame, course, and release", () => {
  ...
  assert.equal(isCanonicalProgressMessage({ ...event, origin: "https://example.test" }, frame), false);
  assert.equal(isCanonicalProgressMessage({ ...event, source: {} }, frame), false);
  assert.equal(isCanonicalProgressMessage({ ...event, data: { ...event.data, releaseId: "old" } }, frame), false);
});

test("the two real course paths normalize into separate evidence writes", () => {
  const rows = normalizeEmbeddedProgress({ progress: {
...
test("assignment evidence selects the first unfinished canonical unit", () => {
  const assignment = { units: [
    { unit_id: "ep01", completed: true },
    { unit_id: "ep02", completed: false },
    { unit_id: "q01", completed: false },
  ] };
  assert.deepEqual(assignmentProgressSummary(assignment), { completed: 1, total: 3, percent: 33 });
...
test("catalog groups preserve Foundations before AI Quest", () => {
  const groups = groupCanonicalUnits([
    ...
    { unit_id: "ep01", path: "foundations", group_number: 1, group_title: "Foundation" },
  ]);
  assert.deepEqual(groups.map((group) => group.path), ["foundations", "ai-quest"]);
});

test("approved instrument definitions drive the response form", () => {
  assert.deepEqual(instrumentQuestions({ definition: { questions: [{ key: "confidence", label: "Confidence", type: "number" }] } }), [
    { key: "confidence", label: "Confidence", help: "", type: "number", options: [], required: true },
  ]);
});
