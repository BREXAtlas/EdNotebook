...
export const DIGITAL_LITERACY_PROGRESS_MESSAGE =
  "ednotebook.digital-literacy.progress.v1";
...
export function buildCanonicalUnitUrl({ assignment, unit, parentOrigin }) {
  ...
  destination.searchParams.set(
    "ednotebook_assignment",
    assignment.assignment_id,
  );
  return destination.toString();
}

export function isCanonicalProgressMessage(
  event,
  frameWindow,
  expectedReleaseId = DIGITAL_LITERACY_RELEASE_ID,
) {
  return (
    event?.origin === DIGITAL_LITERACY_SOURCE_ORIGIN &&
    event?.source === frameWindow &&
    event?.data?.type === DIGITAL_LITERACY_PROGRESS_MESSAGE &&
    event?.data?.releaseId === expectedReleaseId &&
...
export function normalizeEmbeddedProgress(data) {
  ...
  return [
    {
      path: "foundations",
      completedNodeIds: Array.isArray(foundations.completedNodeIds)
        ? foundations.completedNodeIds
        : [],
      stars:
        foundations.stars && typeof foundations.stars === "object"
          ? foundations.stars
          : {},
    },
    {
      path: "ai-quest",
      completedNodeIds: Array.isArray(aiQuest.completedNodeIds)
        ? aiQuest.completedNodeIds
        : [],
      stars:
        aiQuest.stars && typeof aiQuest.stars === "object" ? aiQuest.stars : {},
    },
  ];
}
  ...
  if (Array.isArray(definition.questions) && definition.questions.length) {
    return definition.questions
      ...
      .map((question) => ({
        ...
        help: question.help || "",
        type: ["select", "radio", "textarea", "number", "text"].includes(
          question.type,
        )
          ? question.type
          : "text",
