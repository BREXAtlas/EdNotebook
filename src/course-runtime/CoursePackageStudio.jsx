...
function ProgressTable({ courseId, publicationId, educatorLabel = "Professor" }) {
...
export default function CoursePackageStudio({ session, onBack, onOpenStudentCourse }) {
  ...
  const earlyPrep = activeCourse?.education_division === "k12";
  ...
  async function saveDraft() {
    ...
    setBusy(false); if (result.error) setNotice(result.error.message); else { setPublication(result.data); setNotice(`${entityTitle} package saved. Students cannot see this draft yet.`); }
  }

  async function publish() {
    if (!validation.valid) { setNotice(validation.errors.join(" ")); return; }
    setBusy(true); setNotice("");
    const result = await publishCoursePackage(courseId, manifest, { displayMode: publication?.display_mode || "full_course", themePreset: manifest.preset.id, gradingMode: manifest.grading.mode, changeSummary: publication?.current_version ? `Updated ${entity} package` : `Initial ${entity} publication` });
    setBusy(false); if (result.error) setNotice(result.error.message); else { setPublication(result.data); setNotice(`${entityTitle} published as version ${result.data.current_version}. Enrollment, student access, progress, and gradebook sync are active.`); }
  }

  async function unpublish() {
    if (!publication?.id) return;
    const result = await setPublicationState(publication.id, publication.status === "published" ? "unpublished" : "published");
    if (result.error) setNotice(result.error.message); else { setPublication(result.data); setNotice(result.data.status === "published" ? `${entityTitle} restored at the same student address.` : `${entityTitle} unpublished. Records and progress were preserved.`); }
  }
  ...
  return <main className="course-output-studio">
    <header className="course-studio-topbar"><button type="button" onClick={onBack}>← {entityTitle} builder</button><div><span>{entityTitle.toUpperCase()} OUTPUT STUDIO</span><strong>Build once. Publish inside EdNotebook.</strong></div><label>{entityTitle}<select value={courseId} onChange={(event) => setCourseId(event.target.value)}>{courses.map((course) => <option value={course.id} key={course.id}>{course.course_code || "COURSE"} · {course.title}</option>)}</select></label></header>
    <nav className="course-studio-tabs">{[["course",entityTitle],["lessons","Lessons"],["resources","Media & resources"],["assignments","Assignments"],["progress","Progress & grading"]].map(([id,label]) => <button className={tab === id ? "is-active" : ""} type="button" onClick={() => setTab(id)} key={id}>{label}</button>)}</nav>
    {notice && <div className="studio-notice" role="status">{notice}</div>}

    {tab === "course" && <div className="studio-two-column"><section className="studio-card"><span className="studio-kicker">{entityTitle.toUpperCase()} IDENTITY</span><h1>Prepare the student destination.</h1><label>{entityTitle} title<input value={manifest.course.title} onChange={(event) => updateCourse("title", event.target.value)} /></label><label>Subtitle<input value={manifest.course.subtitle} onChange={(event) => updateCourse("subtitle", event.target.value)} /></label><label>Description<textarea rows="4" value={manifest.course.description} onChange={(event) => updateCourse("description", event.target.value)} /></label><div className="studio-field-grid"><label>{entityTitle} code<input value={manifest.course.courseCode} onChange={(event) => updateCourse("courseCode", event.target.value)} /></label><label>Subject<input value={manifest.course.subject} onChange={(event) => updateCourse("subject", event.target.value)} /></label></div><label>Learner audience<input value={manifest.course.audience} onChange={(event) => updateCourse("audience", event.target.value)} /></label></section><section className="studio-card"><span className="studio-kicker">PUBLISHING RULES</span><h2>Choose how the {entity} appears and grades.</h2><label>{entityTitle} look<select value={manifest.preset.id} onChange={(event) => setManifest({ ...manifest, preset: { id: event.target.value, version: "1.0" } })}>{Object.values(COURSE_PRESETS).map((preset) => <option key={preset.id} value={preset.id}>{preset.label}</option>)}</select></label><label>Grading<select value={manifest.grading.mode} onChange={(event) => updateGrading("mode", event.target.value)}><option value="auto">Automatic from {entity} checks</option><option value="manual">{entityTitle} completion awaits {educatorLabel.toLowerCase()} grade</option><option value="mixed">Automatic {entity} score plus {educatorLabel.toLowerCase()} assignments</option></select></label><label>Completion points<input type="number" min="1" max="1000" value={manifest.grading.maxPoints} onChange={(event) => updateGrading("maxPoints", Number(event.target.value))} /></label><label>{entityTitle} due date<input type="datetime-local" value={manifest.grading.dueAt ? manifest.grading.dueAt.slice(0,16) : ""} onChange={(event) => updateGrading("dueAt", event.target.value ? new Date(event.target.value).toISOString() : "")} /></label><div className="studio-publish-status"><span>{publication?.status || "draft"}</span><strong>{publication?.current_version ? `Version ${publication.current_version}` : "Not published"}</strong>{publication?.id && <code>ednotebook.com/#/student/{earlyPrep ? "k12" : "university"}/course/{publication.id}</code>}</div></section></div>}
        ...
        <Suspense fallback={<p role="status">Opening governed lesson review…</p>}>
          <LessonDraftReview
            ...
            onAccept={(nextManifest) => {
              ...
              setNotice(
                `${educatorLabel}-accepted lesson added to the existing unpublished ${entity}-package draft. Save draft when ready.`,
              );
            }}
          />
        </Suspense>
    ...
    {tab === "progress" && <ProgressTable courseId={courseId} publicationId={publication?.id} educatorLabel={educatorLabel} />}

    <footer className="course-studio-actions"><div>{validation.valid ? <span className="is-valid">✓ {entityTitle} package is ready to publish</span> : <span>{validation.errors.length} item{validation.errors.length === 1 ? "" : "s"} need attention</span>}</div><button type="button" onClick={() => setPreview(true)}>Preview as student</button><button type="button" disabled={busy} onClick={saveDraft}>Save draft</button><button className="primary" type="button" disabled={busy || !validation.valid} onClick={publish}>{busy ? "Saving…" : publication?.status === "published" ? "Publish updated version" : `Publish ${entity}`}</button>{publication?.id && <button type="button" onClick={unpublish}>{publication.status === "published" ? "Unpublish" : "Restore"}</button>}{publication?.status === "published" && <button type="button" onClick={() => onOpenStudentCourse(publication.id, activeCourse.education_division)}>Open student {entity}</button>}</footer>
