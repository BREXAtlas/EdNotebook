...
export default function CoursePackageStudio({ session, onBack, onOpenStudentCourse }) {
  ...
  return <main className="course-output-studio">
    ...
    <nav className="course-studio-tabs">{[["course","Course"],["lessons","Lessons"],["assignments","Assignments"],["progress","Progress & grading"]].map(([id,label]) => <button className={tab === id ? "is-active" : ""} type="button" onClick={() => setTab(id)} key={id}>{label}</button>)}</nav>
