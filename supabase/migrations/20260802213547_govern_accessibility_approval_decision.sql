-- Govern the accountable accessibilityApproval decision without enabling production.
-- The decision is append-only, exact-candidate-bound, and requires active
-- institutional oversight plus explicit accessibility authority. Platform
-- ownership alone and automated checks alone are intentionally insufficient.
...
begin
  ...
  if p_gate_key='securityApproval' then
  ...
  elsif p_gate_key='accessibilityApproval' then
    if not exists (
      select 1
      from public.institution_memberships membership
      where membership.institution_id=p_institution_id
      ...
      raise exception 'Active institutional oversight membership is required. Platform ownership alone is insufficient';
    end if;
    if not p_attestation then raise exception 'Accountable accessibility reviewer attestation required'; end if;
    if p_tested_commit is null
       or char_length(trim(coalesce(p_migration_version,'')))<8
       or char_length(trim(coalesce(p_environment_reference,'')))<8
       or char_length(trim(coalesce(p_region,'')))<2
       or p_expires_at is null then
      raise exception 'Accessibility decision candidate binding and expiration are required';
    end if;
    if p_tested_commit<>'04927a1a6a286aeee0c0c6b273325521f1754727'
       or p_migration_version<>'20260802210945_govern_security_approval_decision'
       or trim(coalesce(p_evidence_summary->>'evidence_packet_commit',''))<>'e5ca08749a7621ce6cc59df0530d4ef7e13f5e53'
       or trim(coalesce(p_evidence_summary->>'staging_project_ref',''))<>'gfalgonektwdylsxsgzc' then
      raise exception 'Accessibility decision does not bind the approved candidate and evidence packet';
    end if;
    if char_length(trim(coalesce(p_evidence_summary->>'reviewer_name','')))<2
       or char_length(trim(coalesce(p_evidence_summary->>'reviewer_title_and_accessibility_authority','')))<8
       or p_evidence_summary->>'technical_evidence_packet' not like '%ACCESSIBILITY_APPROVAL_EVIDENCE_PACKET.md' then
      raise exception 'Accountable accessibility reviewer identity, authority, and evidence packet are required';
    end if;
    if lower(coalesce(p_evidence_summary->>'decision',''))<>p_status
       or p_evidence_summary->>'candidate_merge_commit'<>p_tested_commit
       or p_evidence_summary->>'hosted_migration'<>p_migration_version
       or p_evidence_summary->>'environment_scope'<>'staging'
       or coalesce((p_evidence_summary->>'reviewer_authority_attested')::boolean,false) is not true
       or coalesce((p_evidence_summary->>'automated_checks_only')::boolean,true) is not false
       or coalesce((p_evidence_summary->>'conformance_claim_made')::boolean,true) is not false
       or coalesce((p_evidence_summary->>'production_project_touched')::boolean,true) is not false
       or coalesce((p_evidence_summary->>'production_student_intake_enabled')::boolean,true) is not false
       or coalesce((p_evidence_summary->>'production_action_executed')::boolean,true) is not false then
      raise exception 'Accessibility decision metadata does not match the governed staging boundary';
    end if;
    v_staging_project_ref := trim(coalesce(p_evidence_summary->>'staging_project_ref',''));
    if position(lower(v_staging_project_ref) in lower(p_environment_reference))=0 then
      raise exception 'Accessibility decision staging project binding is invalid';
    end if;
    if p_status='passed' and (
      coalesce((p_evidence_summary->>'complete_process_review_completed')::boolean,false) is not true
      or coalesce((p_evidence_summary->>'keyboard_and_assistive_technology_reviewed')::boolean,false) is not true
      or coalesce((p_evidence_summary->>'visual_and_responsive_reviewed')::boolean,false) is not true
      or coalesce((p_evidence_summary->>'media_and_content_reviewed')::boolean,false) is not true
      or coalesce((p_evidence_summary->>'remediation_ownership_accepted')::boolean,false) is not true
      or coalesce((p_evidence_summary->>'third_party_boundary_accepted')::boolean,false) is not true
    ) then
      raise exception 'PASS requires complete-process, assistive-technology, visual, media, remediation, and third-party review';
    end if;
    ...
    with current_evidence as (
      ...
      from public.student_data_intake_evidence_versions evidence
      where evidence.institution_id=p_institution_id
        and evidence.gate_key<>'accessibilityApproval'
      order by evidence.gate_key,evidence.version desc
    )
    select min(current_evidence.expires_at)
    into v_evidence_expiration_ceiling
    ...
    if v_evidence_expiration_ceiling is not null
       and p_expires_at>v_evidence_expiration_ceiling then
      raise exception 'Accessibility decision expiry exceeds the underlying evidence ceiling';
    end if;
  end if;
...
comment on function public.record_student_data_intake_evidence(uuid,text,text,text,text,text,text,text,text,jsonb,timestamptz,boolean) is
  'Appends governed intake evidence. securityApproval retains its active institution security-reviewer boundary. accessibilityApproval requires active institutional oversight, exact staging and packet binding, accountable authority, manual complete-process evidence for PASS, and bounded expiration. Neither gate enables production intake.';
